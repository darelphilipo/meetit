...
The Meetit app stores server-side logs in Redis (`meetit:server_logs`) for development and ops debugging. These logs may contain sensitive information (usernames, event IDs, RSVP actions, contact-presence flags, error messages). The `/api/server-logs` endpoint and the in-app debug panel expose these logs and MUST be restricted to users with mod access to the subreddit.
