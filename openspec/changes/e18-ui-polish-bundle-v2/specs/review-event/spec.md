# Spec — Review Event Card (Submit Wizard Card 4)

## Purpose

When a user submits an event, the wizard's final card shows a read-only preview of all entered data so the user can confirm before submission. This spec covers the review card's layout and behavior, particularly the description display.
