...
## Purpose
Allows users to add a Meetit event to their Google Calendar via a deep link button on the event details overlay and the RSVP success card. The link opens Google Calendar's "Add Event" page with the event's title, date, time, location, description, and Google Maps URL prefilled.
