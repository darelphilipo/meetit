...
var editEventId: string | null = null;
...
function openDetailsOverlay(d: { event: any; rsvpCount: number; hasRsvped: boolean; settings: any }) {
  ...
  var s4 = d.hasRsvped
    ...
    : '<div class="detail-card" style="position:absolute;top:0;left:0;right:0;bottom:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;padding:20px;text-align:center;">' +
      ...
      '</div>';
  if (d.isOrganizer) {
    s4 = '<div class="detail-card" style="position:absolute;top:0;left:0;right:0;bottom:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;padding:20px;text-align:center;">' +
      '<div style="font-size:48px;">✏️</div>' +
      '<div style="font-size:18px;font-weight:700;">You\'re the organizer</div>' +
      '<div style="font-size:14px;color:var(--muted);">' + d.rsvpCount + ' people going</div>' +
      '<button class="btn btn-white btn-edit-event" data-id="' + e.id + '" data-action="edit-event" style="width:80%;margin-top:4px;">✏️ Edit Event</button>' +
      '</div>';
  }
...
async function submitEvent() { log("submitEvent editId=" + editEventId); var title = (document.getElementById("event-title") as HTMLInputElement).value.trim(); var organizer = (document.getElementById("event-organizer") as HTMLInputElement).value.trim(); var date = (document.getElementById("event-date") as HTMLInputElement).value.trim(); var time = (document.getElementById("event-time") as HTMLInputElement).value.trim(); var loc = (document.getElementById("event-location") as HTMLInputElement).value.trim(); var mapUrl = (document.getElementById("event-map-url") as HTMLInputElement).value.trim(); var desc = (document.getElementById("event-desc") as HTMLTextAreaElement).value.trim(); if (!title || !organizer || !date || !time || !loc || !desc) { showToast("Fill all fields", "error"); return; } var isEdit = !!editEventId; setBtnLoading("#event-submit-btn", true, isEdit ? "⏳ Saving..." : "⏳ Submitting..."); try { if (isEdit) { await fetch(API_BASE + "/api/update-event", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ eventId: editEventId, title: title, organizer: organizer, date: date, time: time, location: loc, mapUrl: mapUrl, desc: desc }) }); showToast("Event updated! ✅", "success"); delete detailCache[editEventId!]; } else { await fetch(API_BASE + "/api/submit-event", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: title, organizer: organizer, date: date, time: time, location: loc, mapUrl: mapUrl, desc: desc }) }); showToast("Event submitted! ✅", "success"); } closeOverlay("event-overlay"); showHomePage(); } catch (e) { showToast(isEdit ? "Update failed" : "Error", "error"); setBtnLoading("#event-submit-btn", false); } }
function openEditEvent(id: string) { log("openEditEvent id=" + id); var evt = cachedHomeEvents.find(function (e) { return e.id === id; }) || { title: "", date: "", time: "", location: "", description: "", organizer: "", mapUrl: "" }; editEventId = id; resetEventForm(); (document.getElementById("event-title") as HTMLInputElement).value = evt.title || ""; (document.getElementById("event-organizer") as HTMLInputElement).value = evt.organizer || ""; (document.getElementById("event-date") as HTMLInputElement).value = evt.date || ""; (document.getElementById("event-time") as HTMLInputElement).value = evt.time || ""; (document.getElementById("event-location") as HTMLInputElement).value = evt.location || ""; (document.getElementById("event-map-url") as HTMLInputElement).value = evt.mapUrl || ""; (document.getElementById("event-desc") as HTMLTextAreaElement).value = evt.description || ""; document.getElementById("event-overlay")!.querySelector("h2")!.textContent = "✏️ Edit Event"; openOverlay("event-overlay"); }
...
function handleAction(action: string, id: string | null) {
  switch (action) {
    ...
    case "edit-event": if (id) openEditEvent(id); break;
