const state: Record<string, any> = {
  ...
  eventStep: 1,
  detailStep: 1,
  detailStep1: "", detailStep2: "", detailStep3: "", detailStep4: "",
  homeCardIndex: 0,
  cachedHomeEvents: [],
  ...
  myPitches: [],
  myEvents: [],
  CACHE_TTL_HOME: 30000,
  CACHE_TTL_DETAIL: 30000,
  CACHE_TTL_ATTENDEES: 30000,
  CACHE_TTL_MOD: 60000,
  homeFetchTimeout: null,
  detailCache: {},
  attendeeCache: {},
  modTabCache: {},
...
async function loadHome() {
  ...
  if (state.homeFetchTimeout) clearTimeout(state.homeFetchTimeout);
  state.homeFetchTimeout = setTimeout(async function () {
    var loadSeq = ++state.homeLoadSeq;
    try {
      var res = await fetch(state.API_BASE + "/api/home");
      if (loadSeq !== state.homeLoadSeq) return;
      ...
      if (data.type === "home") {
        ...
        var currentEvent = state.cachedHomeEvents[state.homeCardIndex];
        var currentId = currentEvent && currentEvent.id;
        state.cachedHomeEvents = allEvents;
        state.cachedHomeIsMod = data.data.isMod;
        if (currentId) {
          var updatedIndex = allEvents.findIndex(function (event) { return event.id === currentId; });
          if (updatedIndex >= 0) state.homeCardIndex = updatedIndex;
          else if (state.homeCardIndex >= allEvents.length) state.homeCardIndex = Math.max(0, allEvents.length - 1);
        } else if (state.homeCardIndex >= allEvents.length) {
          state.homeCardIndex = Math.max(0, allEvents.length - 1);
        }
        setTimeout(function () {
          if (loadSeq === state.homeLoadSeq) renderHomeCard(data.data);
        }, 200);
      }
...
function renderHomeCard(state: { eventsByDate: Record<string, any[]>; isMod: boolean; settings: any }) {
  ...
  if (dates.length === 0) {
    ...
    state.cachedHomeEvents = all;
    state.cachedHomeIsMod = state.isMod;
    if (state.homeCardIndex >= all.length) state.homeCardIndex = 0;
    var event = all[state.homeCardIndex];
    if (!event) return;
    ...
    c.innerHTML =
      ...
      '<span style="font-size:11px;font-weight:700;background:var(--surface);border:var(--border);padding:3px 10px;">' + (state.homeCardIndex + 1) + '/' + count + '</span>' +
...
function homePrev() { log("homePrev idx=" + state.homeCardIndex + " total=" + state.cachedHomeEvents.length); if (state.cachedHomeEvents.length > 1) { state.homeCardIndex = (state.homeCardIndex - 1 + state.cachedHomeEvents.length) % state.cachedHomeEvents.length; renderHomeCard({ eventsByDate: groupByDate(state.cachedHomeEvents), isMod: state.cachedHomeIsMod, settings: {} }); } }
function homeNext() { log("homeNext idx=" + state.homeCardIndex + " total=" + state.cachedHomeEvents.length); if (state.cachedHomeEvents.length > 1) { state.homeCardIndex = (state.homeCardIndex + 1) % state.cachedHomeEvents.length; renderHomeCard({ eventsByDate: groupByDate(state.cachedHomeEvents), isMod: state.cachedHomeIsMod, settings: {} }); } }
...
async function loadMySubmissions() {
  ...
  try {
    var res = await fetch(state.API_BASE + "/api/my-submissions");
    var data = await res.json();
    if (data.type === "my-submissions") {
      state.myPitches = data.pitches || [];
      state.myEvents = data.events || [];
      state.myPitchIdx = 0; state.myEventIdx = 0;
      renderMyPitchCard();
      renderMyEventCard();
    }
  } catch (e) {
  ...
  state.myStuffLoading = false;
}
function renderMyPitchCard() {
  var el = document.getElementById("my-pitches-section")!;
  if (state.myPitches.length === 0) { el.innerHTML = '<div style="text-align:center;padding:8px;color:var(--muted);font-size:13px;">💡 No pitches</div>'; return; }
  if (state.myPitchIdx >= state.myPitches.length) state.myPitchIdx = 0;
  var p = state.myPitches[state.myPitchIdx];
  var total = state.myPitches.length;
  el.innerHTML = '<div class="idea-card" style="height:100%;display:flex;flex-direction:column;padding:10px;overflow:hidden;box-sizing:border-box;">' +
    ...
    '<div style="flex-shrink:0;display:flex;gap:6px;justify-content:center;align-items:center;padding-top:6px;">' +
      ...
      (total > 1 ? '<button class="btn btn-white btn-sm btn-my-pitch-prev" data-action="my-pitch-prev" style="padding:4px 10px;font-size:11px;">←</button><span style="font-size:11px;font-weight:700;">' + (state.myPitchIdx + 1) + '/' + total + '</span><button class="btn btn-white btn-sm btn-my-pitch-next" data-action="my-pitch-next" style="padding:4px 10px;font-size:11px;">→</button>' : '') +
    '</div></div>';
  
}
function renderMyEventCard() {
  var el = document.getElementById("my-events-section")!;
  if (state.myEvents.length === 0) { el.innerHTML = '<div style="text-align:center;padding:8px;color:var(--muted);font-size:13px;">📋 No events</div>'; return; }
  if (state.myEventIdx >= state.myEvents.length) state.myEventIdx = 0;
  var e = state.myEvents[state.myEventIdx];
  ...
  el.innerHTML = '<div class="pending-card" style="height:100%;display:flex;flex-direction:column;padding:10px;overflow:hidden;box-sizing:border-box;background:' + bg + ';">' +
    ...
    '<div style="flex-shrink:0;display:flex;justify-content:center;align-items:center;gap:6px;padding-top:6px;">' +
      (total > 1 ? '<button class="btn btn-white btn-sm btn-my-event-prev" data-action="my-event-prev" style="padding:4px 10px;font-size:11px;">←</button><span style="font-size:11px;font-weight:700;">' + (state.myEventIdx + 1) + '/' + total + '</span><button class="btn btn-white btn-sm btn-my-event-next" data-action="my-event-next" style="padding:4px 10px;font-size:11px;">→</button>' : '') +
    '</div></div>';
  
}
function myPitchNext() { state.myPitchIdx++; if (state.myPitchIdx >= state.myPitches.length) state.myPitchIdx = 0; renderMyPitchCard(); }
function myPitchPrev() { state.myPitchIdx--; if (state.myPitchIdx < 0) state.myPitchIdx = state.myPitches.length - 1; renderMyPitchCard(); }
function myEventNext() { state.myEventIdx++; if (state.myEventIdx >= state.myEvents.length) state.myEventIdx = 0; renderMyEventCard(); }
function myEventPrev() { state.myEventIdx--; if (state.myEventIdx < 0) state.myEventIdx = state.myEvents.length - 1; renderMyEventCard(); }
...
function renderAttendees(eventId: string, att: any[]) {
  ...
  if (state.detailStep === 3) persistStep3(eventId);
}
async function loadPublicAttendees(eventId: string) {
  // Check cache first
  var cached = state.attendeeCache[eventId];
  if (cached && Date.now() - cached.timestamp < state.CACHE_TTL_ATTENDEES) {
    renderAttendees(eventId, cached.data);
    return;
  }
  try {
    var res = await fetch(state.API_BASE + "/api/rsvp-list", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ eventId: eventId }) });
    var data = await res.json();
    if (data.type === "rsvp-list") {
      var att = data.attendees || [];
      state.attendeeCache[eventId] = { data: att, timestamp: Date.now() };
...
function buildAttNav(eventId: string): string {
  var total = Math.ceil((state.attStore[eventId] || []).length / 5);
  if (total <= 1) return '';
  var cur = state.attPageMap[eventId] || 0;
...
function splitTextToPages(text: string, width: number, maxHeight: number): string[] {
  if (!state._measureDiv) {
    state._measureDiv = document.createElement("div");
    state._measureDiv.style.cssText = "position:absolute;left:-9999px;top:0;font-size:15px;line-height:1.5;font-family:'Space Grotesk',sans-serif;padding:14px;word-break:break-word;white-space:pre-wrap;visibility:hidden;";
    document.body.appendChild(state._measureDiv);
  }
  state._measureDiv.style.width = width + "px";
  var words = text.split(" ");
  var pages: string[] = [];
  var current = "";
  for (var i = 0; i < words.length; i++) {
    var test = current ? current + " " + words[i] : words[i];
    state._measureDiv.textContent = test;
    if (state._measureDiv.scrollHeight > maxHeight && current) { pages.push(current); current = words[i]; }
    else { current = test; }
  }
...
function buildDescNavHTML(eventId: string): string {
  var total = state.descPageTotal[eventId] || 1;
  if (total <= 1) return '';
  var cur = state.descPageMap[eventId] || 0;
...
function persistStep2(eventId: string) {
  ...
  if (!body) return;
  state.detailStep2 = body.innerHTML;
}

function persistStep3(eventId: string) {
  var body = document.getElementById("detail-body");
  if (!body) return;
  state.detailStep3 = body.innerHTML;
}

async function showEventDetails(id: string) {
  log("showEventDetails id=" + id + " loading=" + state.detailLoading);
  if (state.detailLoading) return;

  // Check detail cache before fetching
  var cached = state.detailCache[id];
  if (cached && Date.now() - cached.timestamp < state.CACHE_TTL_DETAIL) {
    log("showEventDetails using state.detailCache " + id);
    openDetailsOverlay(cached.data);
    // Still set state.detailLoading to block concurrent calls, but don't fetch
  ...
  var cachedEvent = state.cachedHomeEvents.find(function (event) { return event.id === id; });
  if (cachedEvent) {
  ...
  try {
    var res = await fetch(state.API_BASE + "/api/event-details", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ eventId: id }) });
    var data = await res.json();
    if (data.type === "event-details" && state.currentEventId === id) {
      log("showEventDetails server response " + id + " hasRsvped=" + data.data.hasRsvped);
      state.detailCache[id] = { data: data.data, timestamp: Date.now() };
      openDetailsOverlay(data.data);
      state.detailLoading = false;
      return;
    }
    log("showEventDetails stale response " + id + " current=" + state.currentEventId);
...
function openDetailsOverlay(d: { event: any; rsvpCount: number; hasRsvped: boolean; settings: any }) {
  ...
  log("openDetailsOverlay event=" + e.id + " isOpen=" + isOpen + " hasRsvped=" + d.hasRsvped + " step=" + state.detailStep);
  ...
  state.descFullStore[e.id] = descFull;
  if (!isOpen) { state.descPageMap[e.id] = 0; state.descPageTotal[e.id] = hasMore ? 99 : 1; }
  ...
  // Card 3: Who's Going
  if (!isOpen) state.attPageMap[e.id] = 0;
  ...
  state.detailStep1 = s1; state.detailStep2 = s2; state.detailStep3 = s3; state.detailStep4 = s4;
  if (!isOpen) {
    state.detailStep = 1;
...
function detailNext() {
  log("detailNext from=" + state.detailStep);
  if (state.detailStep === 1) {
    document.getElementById("detail-dot-2")!.classList.add("done");
    document.getElementById("detail-body")!.innerHTML = state.detailStep2;
    document.getElementById("detail-prev-btn")!.classList.remove("hidden");
     state.detailStep = 2;
  } else if (state.detailStep === 2) {
    document.getElementById("detail-dot-3")!.classList.add("done");
    document.getElementById("detail-body")!.innerHTML = state.detailStep3;
    if (state.currentEventId) loadPublicAttendees(state.currentEventId);
     state.detailStep = 3;
  } else if (state.detailStep === 3) {
    document.getElementById("detail-dot-4")!.classList.add("done");
    document.getElementById("detail-body")!.innerHTML = state.detailStep4;
...
function detailPrev() {
  log("detailPrev from=" + state.detailStep);
  if (state.detailStep === 2) {
    document.getElementById("detail-dot-2")!.classList.remove("done");
    document.getElementById("detail-body")!.innerHTML = state.detailStep1;
...
function modNext(tab: string) { log("modNext tab=" + tab); var idx = (state.modCardIndex[tab] || 0) + 1; state.modCardIndex[tab] = idx; renderModCard(tab); }
function modPrev(tab: string) { log("modPrev tab=" + tab); var idx = (state.modCardIndex[tab] || 0) - 1; if (idx < 0) idx = 0; state.modCardIndex[tab] = idx; renderModCard(tab); }
function showModDashboard() { openOverlay("mod-screen"); loadModTab("pending"); }
function switchModTab(tab: string) { if (tab === state.modTab) return; state.modTab = tab; document.querySelectorAll("#mod-tabs .mod-tab").forEach(function (t) { t.classList.toggle("active", (t as HTMLElement).dataset.mtab === tab); }); loadModTab(tab); }
...
async function loadModTab(tab: string) {
  // Check cache first
  var cached = state.modTabCache[tab];
  if (cached && Date.now() - cached.timestamp < state.CACHE_TTL_MOD) {
  ...
  setModLoading(true);
  if (tab === "pending") { try { var pr = await fetch(state.API_BASE + "/api/pending-events"); var pd = await pr.json(); if (pd.type === "pending-events") { state.modTabCache[tab] = { data: pd.events, timestamp: Date.now() }; renderModPending(pd.events); } } catch (e) { console.error(e); } } else if (tab === "published") { try { var res = await fetch(state.API_BASE + "/api/all-approved-events"); var d = await res.json(); if (d.type === "all-approved-events") { state.modTabCache[tab] = { data: d.events, timestamp: Date.now() }; renderModPublished(d.events); } } catch (e) { console.error(e); } } else if (tab === "pitches") { try { var res = await fetch(state.API_BASE + "/api/pitched-ideas"); var d = await res.json(); if (d.type === "pitched-ideas") { state.modTabCache[tab] = { data: d.ideas, timestamp: Date.now() }; renderModPitches(d.ideas); } } catch (e) { console.error(e); } }
  setModLoading(false);
}
function renderModCard(tab: string) {
  var items = state.modItems[tab] || [];
  var idx = state.modCardIndex[tab] || 0;
  if (items.length === 0) return;
  if (idx >= items.length) { state.modCardIndex[tab] = 0; idx = 0; }
  var item = items[idx];
  ...
  state.modDescFull[dcKey] = desc;
  if (!state.modDescTotal[dcKey]) state.modDescTotal[dcKey] = desc.length > 100 ? 99 : 1;
  state.modDescPage[dcKey] = 0;
  ...
  if (desc.length > 100) {
    setTimeout(function () {
      ...
      if (!box) return;
      var pages = splitTextToPages(state.modDescFull[dcKey2] || "", box.clientWidth, box.clientHeight);
      state.modDescTotal[dcKey2] = pages.length;
      state.modDescPage[dcKey2] = 0;
...
function buildModDescNavHTML(key: string): string {
  var total = state.modDescTotal[key] || 1;
  if (total <= 1) return '';
  var cur = state.modDescPage[key] || 0;
...
function bindModDescNav(key: string) {
  document.querySelectorAll("#mod-desc-nav-" + key + " .btn-mod-desc-next").forEach(function (b) { b.addEventListener("click", function () { var cur = (state.modDescPage[key] || 0) + 1; if (cur >= (state.modDescTotal[key] || 1)) return; state.modDescPage[key] = cur; slideTrack("mod-desc-track-" + key, cur, state.modDescTotal[key] || 1); document.getElementById("mod-desc-nav-" + key)!.innerHTML = buildModDescNavHTML(key); bindModDescNav(key); }); });
  document.querySelectorAll("#mod-desc-nav-" + key + " .btn-mod-desc-prev").forEach(function (b) { b.addEventListener("click", function () { var cur = (state.modDescPage[key] || 0) - 1; if (cur < 0) return; state.modDescPage[key] = cur; slideTrack("mod-desc-track-" + key, cur, state.modDescTotal[key] || 1); document.getElementById("mod-desc-nav-" + key)!.innerHTML = buildModDescNavHTML(key); bindModDescNav(key); }); });
}

function renderModPending(events: any[]) {
  state.modItems["pending"] = events;
  state.modCardIndex["pending"] = 0;
  if (events.length === 0) {
...
function renderModPublished(events: any[]) {
  state.modItems["published"] = events;
  state.modCardIndex["published"] = 0;
  if (events.length === 0) {
...
function renderModPitches(ideas: any[]) {
  state.modItems["pitches"] = ideas;
  state.modCardIndex["pitches"] = 0;
  if (ideas.length === 0) {
...
// ======= ACTIONS =======
function isLocked(key: string): boolean { return !!state.actionLocks[key]; }
function lock(key: string) { state.actionLocks[key] = true; }
function unlock(key: string) { state.actionLocks[key] = false; }
async function approveEvent(id: string) { log("approveEvent id=" + id); var k = "approve-" + id; if (isLocked(k)) return; lock(k); var title = getItemTitle(id, state.modItems); if (!await confirmDestructive('Approve "' + title + '"? This will publish it for everyone.')) { unlock(k); return; } var btn = document.querySelector('[data-id="' + id + '"].btn-approve-event') as HTMLElement; if (btn) { btn.style.opacity = "0.3"; btn.style.pointerEvents = "none"; btn.textContent = "⏳ Approving..."; } try { await fetch(state.API_BASE + "/api/approve-event", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ eventId: id }) }); showToast("Event approved!", "success"); delete state.modTabCache["pending"]; setTimeout(function () { loadModTab("pending"); }, 300); } catch (e) { showToast("Error", "error"); if (btn) { btn.style.opacity = "1"; btn.style.pointerEvents = "auto"; btn.textContent = "✅ Approve & Publish"; } } finally { unlock(k); } }
async function deleteEvent(id: string, type: string) { log("deleteEvent id=" + id + " type=" + type); var k = type + "-" + id; if (isLocked(k)) return; lock(k); var title = getItemTitle(id, state.modItems); if (!await confirmDestructive('Delete "' + title + '"? This cannot be undone.')) { unlock(k); return; } var sel = type === "pending" ? ".btn-decline-event" : ".btn-delete-published"; var btn = document.querySelector('[data-id="' + id + '"]' + sel) as HTMLElement; if (btn) { btn.style.opacity = "0.3"; btn.style.pointerEvents = "none"; } var parent = btn ? btn.closest(".pending-card,.event-card") as HTMLElement : null; if (parent) parent.style.opacity = "0.3"; var endpoint = type === "pending" ? "/api/delete-pending" : "/api/delete-published"; try { await fetch(state.API_BASE + endpoint, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ eventId: id }) }); showToast("Deleted", "success"); delete state.modTabCache[type]; setTimeout(function () { loadModTab(type === "pending" ? "pending" : "published"); }, 300); } catch (e) { showToast("Error", "error"); if (parent) parent.style.opacity = "1"; if (btn) { btn.style.opacity = "1"; btn.style.pointerEvents = "auto"; } } finally { unlock(k); } }
async function dismissIdea(id: string) { log("dismissIdea id=" + id); var title = getItemTitle(id, state.modItems); if (!await confirmDestructive('Dismiss "' + title + '"? This cannot be undone.')) return; setBtnLoading('[data-action="dismiss-idea"][data-id="' + id + '"]', true, "⏳ Dismissing..."); try { await fetch(state.API_BASE + "/api/dismiss-idea", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ideaId: id }) }); showToast("Idea dismissed", "success"); loadModTab("pitches"); } catch (e) { showToast("Error", "error"); setBtnLoading('[data-action="dismiss-idea"][data-id="' + id + '"]', false); } }
async function deletePitch(id: string) { log("deletePitch id=" + id); var title = getItemTitle(id, { myPitches: state.myPitches }); if (!await confirmDestructive('Delete "' + title + '"? This cannot be undone.')) return; var k = "pitch-" + id; if (isLocked(k)) return; lock(k); setBtnLoading('[data-action="delete-pitch"][data-id="' + id + '"]', true, "⏳ Deleting..."); try { await fetch(state.API_BASE + "/api/dismiss-idea", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ideaId: id }) }); showToast("Deleted", "success"); loadMySubmissions(); } catch (e) { showToast("Error", "error"); } finally { setBtnLoading('[data-action="delete-pitch"][data-id="' + id + '"]', false); unlock(k); } }
async function viewRsvps(eventId: string) { log("viewRsvps id=" + eventId);
  ...
  try {
    var res = await fetch(state.API_BASE + "/api/rsvp-list", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ eventId: eventId, includeContactDetails: true }) });
    var data = await res.json();
    if (data.type === "rsvp-list") {
      var att = data.attendees || [];
...
async function submitRsvp() {
  log("submitRsvp eventId=" + state.currentEventId);
  ...
  try {
    await fetch(state.API_BASE + "/api/rsvp", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ eventId: state.currentEventId, email: email, phone: phone }) });
    showToast("RSVP confirmed! 🎉", "success");
    delete state.detailCache[state.currentEventId];
    delete state.attendeeCache[state.currentEventId];
...
function handleAction(action: string, id: string | null) {
  switch (action) {
    ...
    case "rsvp-card": if (id) { var evt = state.cachedHomeEvents.find(function(e) { return e.id === id; }); if (evt && evt.hasRsvped) showEventDetails(id); else showRsvpOverlay(id); } break;
    case "home-prev": homePrev(); break;
    case "home-next": homeNext(); break;
    case "desc-next": if (id) {
      log("desc-next id=" + id + " pageTotal=" + (state.descPageTotal[id] || 0) + " curPage=" + (state.descPageMap[id] || 0));
      if (state.descPageTotal[id] === 99) {
        log("desc-next PAGINATING id=" + id);
        var box = document.getElementById("desc-box-" + id);
        if (!box) return;
        var pages = splitTextToPages(state.descFullStore[id] || "", box.clientWidth, box.clientHeight);
        log("desc-next split into " + pages.length + " pages id=" + id);
        state.descPageTotal[id] = pages.length;
        state.descPageMap[id] = 0;
        ...
        var cur = (state.descPageMap[id] || 0) + 1;
        if (cur >= (state.descPageTotal[id] || 1)) { log("desc-next BLOCKED at last page id=" + id); return; }
        state.descPageMap[id] = cur;
        log("desc-next slide id=" + id + " page=" + cur + "/" + state.descPageTotal[id]);
        slideTrack("desc-track-" + id, cur, state.descPageTotal[id] || 1);
        document.getElementById("desc-nav-" + id)!.innerHTML = buildDescNavHTML(id);
        persistStep2(id);
      }
    } break;
    case "desc-prev": if (id) {
      var cur2 = (state.descPageMap[id] || 0) - 1;
      if (cur2 < 0) { log("desc-prev BLOCKED at first page id=" + id); return; }
      state.descPageMap[id] = cur2;
      log("desc-prev slide id=" + id + " page=" + cur2 + "/" + state.descPageTotal[id]);
      slideTrack("desc-track-" + id, cur2, state.descPageTotal[id] || 1);
      document.getElementById("desc-nav-" + id)!.innerHTML = buildDescNavHTML(id);
      persistStep2(id);
    } break;
    case "att-next": if (id) { var c3 = (state.attPageMap[id] || 0) + 1; state.attPageMap[id] = c3; var t3 = Math.ceil((state.attStore[id] || []).length / 5); log("att-next id=" + id + " page=" + c3 + "/" + t3); slideTrack("att-track-" + id, c3, t3); document.getElementById("att-nav-" + id)!.innerHTML = buildAttNav(id); persistStep3(id); } break;
    case "att-prev": if (id) { var c4 = (state.attPageMap[id] || 0) - 1; state.attPageMap[id] = c4; var t4 = Math.ceil((state.attStore[id] || []).length / 5); log("att-prev id=" + id + " page=" + c4 + "/" + t4); slideTrack("att-track-" + id, c4, t4); document.getElementById("att-nav-" + id)!.innerHTML = buildAttNav(id); persistStep3(id); } break;
    ...
    case "mod-desc-next": {
      if (!id) break;
      var c5 = (state.modDescPage[id] || 0) + 1;
      if (c5 >= (state.modDescTotal[id] || 1)) return;
      state.modDescPage[id] = c5;
      slideTrack("mod-desc-track-" + id, c5, state.modDescTotal[id] || 1);
      document.getElementById("mod-desc-nav-" + id)!.innerHTML = buildModDescNavHTML(id);
      bindModDescNav(id);
    } break;
    case "mod-desc-prev": {
      if (!id) break;
      var c6 = (state.modDescPage[id] || 0) - 1;
      if (c6 < 0) return;
      state.modDescPage[id] = c6;
      slideTrack("mod-desc-track-" + id, c6, state.modDescTotal[id] || 1);
      document.getElementById("mod-desc-nav-" + id)!.innerHTML = buildModDescNavHTML(id);
      bindModDescNav(id);
    } break;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.getElementById("confirm-ok-btn")!.addEventListener("click", function () { closeOverlay("confirm-overlay"); if (state.confirmResolve) { state.confirmResolve(true); state.confirmResolve = null; } });
  document.getElementById("confirm-cancel-btn")!.addEventListener("click", function () { closeOverlay("confirm-overlay"); if (state.confirmResolve) { state.confirmResolve(false); state.confirmResolve = null; } });
  document.getElementById("confirm-backdrop")!.addEventListener("click", function () { closeOverlay("confirm-overlay"); if (state.confirmResolve) { state.confirmResolve(false); state.confirmResolve = null; } });
