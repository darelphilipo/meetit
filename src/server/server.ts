...
async function onPitchIdea(req: IncomingMessage): Promise<ApiResponse> {
  ...
  if (!title || typeof title !== "string" || title.length > 200) return { type: "pitch-idea", success: false, error: "Invalid title", status: 400 } as any;
  if (!description || typeof description !== "string" || description.length > 2000) return { type: "pitch-idea", success: false, error: "Invalid description", status: 400 } as any;
...
// Helper: post notification comment on the app post (disabled - submitComment not available)
// async function notifyMods... kept for reference
...
async function onCheckEvents(req: IncomingMessage): Promise<TaskResponse> {
  ...
  try {
    ...
    for (const [eventId, eventJson] of Object.entries(allEvents)) {
      ...
      // Set flag BEFORE notification to prevent CRON double-fire race
